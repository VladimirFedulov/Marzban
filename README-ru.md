...
| SUBSCRIPTION_HIDE_DEFAULT_HOSTS_WHEN_CUSTOM_HOSTS | Скрывать стандартные хосты, если есть пользовательские (по умолчанию: `False`)                                       |
...
| SKIP_NODE_DISCONNECT_ON_SHUTDOWN         | Не отключать ноды при остановке/перезапуске Marzban (по умолчанию: `False`)                                                    |
...
## Перезапуск Marzban без отключения нод

Если вы используете `marzban restart`, по умолчанию Marzban отключает ноды при завершении работы. Чтобы перезапустить сам Marzban, не отправляя команду отключения нодам, добавьте переменную окружения `SKIP_NODE_DISCONNECT_ON_SHUTDOWN=true` в файл `.env` (обычно `/opt/marzban/.env`), затем выполните перезапуск:
